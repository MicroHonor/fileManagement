		...
		this.control({
			...
			"topview button[ref=logout]" : {
				click: function(btn){
					...
					for(var i=1;i<=mainView.items.length;i++){
						mainView.items.getAt(i).close();
					}
				}
			},
			...
			"westview treepanel":{
				itemclick:function(tree,record,item,index,e,eOpts){
					...
					if(record.data["id"]=="usermanager"){
						...
						/**户籍管理*/
					}else if(record.data["id"]=="household_catalog"){
						self.addFunItem({
							mainView:mainView,
							funViewXtype:"householdlayout",
							funController:"core.household.controller.HouseholdController",
							funViewName:"core.household.view.HouseholdLayout"
						});	
					}else if(record.data["id"]=="household_inner_catalog"){
						self.addFunItem({
							mainView:mainView,
